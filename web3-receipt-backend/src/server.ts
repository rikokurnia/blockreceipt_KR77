  ...
  try {
    ...
    const newAgreement = await prisma.agreement.create({
      data: {
        ...
        // GUNAKAN ID HASIL FILTER DI ATAS
    ...
    // Tampilkan error detail ke response agar terlihat di Network Tab browser
...
app.get("/api/stats", async (req, res) => {
  try {
    ...
    const pendingInvoices = await prisma.receipt.count({
    ...
    const monthlyTrend = Array.from(trendMap, ([month, amount]) => ({
      month,
      amount,
    }));
  ...
  try {
    ...
    if (!isValid) {
      return res.status(400).json({
        error:
          "Proof Generation Failed: Actual spending data does not satisfy the claim.",
      });
    }
...
app.get("/api/zk/verify/:id", async (req, res) => {
  ...
  try {
    ...
    const sanitizedProof = JSON.parse(
